...
# Path to the new flat catalog CSV
CATALOG_CSV = "product_catalog_2_3_unique (1)(Sheet1).csv"
...
def load_catalog(csv_path: str) -> pd.DataFrame:
    """Load and clean the flat product catalog CSV."""
...
def main():
    ...
    print("=== Starting Ingestion: Flat Catalog V2 → Pinecone ===")
    print(f"  Index : {PINECONE_CATALOG_INDEX_NAME}")
    ...
    # 4. Connect to Pinecone — the index was pre-created by the user
    ...
    if PINECONE_CATALOG_INDEX_NAME in active_indexes:
        existing_dim = pc.describe_index(PINECONE_CATALOG_INDEX_NAME).dimension
        if existing_dim != embedding_dim:
            print(
                f"  Dimension mismatch: index has {existing_dim}D but model outputs {embedding_dim}D. "
                f"Deleting '{PINECONE_CATALOG_INDEX_NAME}' and recreating..."
            )
            pc.delete_index(PINECONE_CATALOG_INDEX_NAME)
            print("  Waiting for deletion to complete...")
            while PINECONE_CATALOG_INDEX_NAME in [idx.name for idx in pc.list_indexes()]:
                time.sleep(2)
            print("  Index deleted.")
            active_indexes = [idx.name for idx in pc.list_indexes()]
        else:
            print(f"  Connected to existing index '{PINECONE_CATALOG_INDEX_NAME}' (dim={existing_dim}).")

    if PINECONE_CATALOG_INDEX_NAME not in active_indexes:
        print(f"Creating serverless index '{PINECONE_CATALOG_INDEX_NAME}' (dim={embedding_dim})...")
        try:
            pc.create_index(
                name=PINECONE_CATALOG_INDEX_NAME,
                dimension=embedding_dim,
                metric="cosine",
                spec=ServerlessSpec(cloud="aws", region="us-east-1"),
            )
            print("Waiting for index to be ready...")
            while not pc.describe_index(PINECONE_CATALOG_INDEX_NAME).status.ready:
                time.sleep(2)
            print("Index ready.")
        except Exception as e:
    ...
    index = pc.Index(PINECONE_CATALOG_INDEX_NAME)
    ...
    print(f"  Pinecone index        : {PINECONE_CATALOG_INDEX_NAME}")
    print("==========================================")
